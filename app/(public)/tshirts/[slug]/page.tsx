/**
 * FILE: app/(public)/tshirts/[slug]/page.tsx
 * ROLE: Public — T-Shirts product detail page, served at
 * "/tshirts/[slug]".
 *
 * PURPOSE:
 ...
 * 404'd its "View Details" link (same gap task-126 fixed for Game
 * Characters). Calls notFound() for any slug that isn't a published
 * T-Shirts product, which renders the sibling not-found.tsx in this
 * route segment.
 *
 * DATA FLOW:
...
const CATEGORY = "tshirts" as const;
...
/**
 * getSiblings
 * Every other published T-Shirts product, for the "More in this
...
export default async function TShirtProductPage({ params }: ProductPageProps) {
  const { slug } = await params;
  const product = await getProduct(slug);

  // No matching published T-Shirts product for this slug — render the not-found segment
  if (!product) {
    notFound();
  }
