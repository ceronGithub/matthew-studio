 ...
 * (scoped to the "tshirts" category) and renders it via the shared
 ...
 * notFound() for any slug that isn't a known T-Shirts product, which
 * renders the sibling not-found.tsx in this route segment.
 *
 * DATA FLOW:
 * 1. generateStaticParams pre-renders one page per known T-Shirts
 *    product slug.
...
// Pre-render a static page for every known T-Shirts product slug
...
export default async function TShirtProductPage({ params }: ProductPageProps) {
  ...
  // No matching T-Shirts product for this slug — render the not-found segment
  if (!product) {
    notFound();
  }
