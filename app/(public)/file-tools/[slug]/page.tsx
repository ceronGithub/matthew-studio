 ...
 * 404'd its "View Details" link (same gap task-126/127 fixed for
 * Game Characters/T-Shirts). Calls notFound() for any slug that isn't
 * a published File Tools product, which renders the sibling
...
/**
 * getSiblings
 * Every other published File Tools product, for the "More in this
...
export default async function FileToolProductPage({ params }: ProductPageProps) {
  ...
  // No matching published File Tools product for this slug — render the not-found segment
  if (!product) {
    notFound();
  }
