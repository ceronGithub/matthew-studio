 ...
 * (scoped to the "tutorials" category) and renders it via the shared
 ...
 * notFound() for any slug that isn't a known Tutorials product, which
 ...
 *
 * DATA FLOW:
 * 1. generateStaticParams pre-renders one page per known Tutorials
 *    product slug.
...
// Pre-render a static page for every known Tutorials product slug
...
export default async function TutorialProductPage({ params }: ProductPageProps) {
  ...
  // No matching Tutorials product for this slug — render the not-found segment
  if (!product) {
    notFound();
  }
