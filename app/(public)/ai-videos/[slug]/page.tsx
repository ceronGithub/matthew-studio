 ...
 * 404'd its "View Details" link (docs/openFindings.md). Calls
 * notFound() for any slug that isn't a published AI Videos product,
 * which renders the sibling not-found.tsx in this route segment.
 *
 * DATA FLOW:
...
/**
 * getSiblings
 * Every other published AI Videos product, for the "More in this
...
export default async function AIVideoProductPage({ params }: ProductPageProps) {
  ...
  // No matching published AI Videos product for this slug — render the not-found segment
  if (!product) {
    notFound();
  }
