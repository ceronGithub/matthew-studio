 ...
 * (scoped to the "game-characters" category) and renders it via the
 * shared ProductDetail component, per improvement_1.md Section 4's
 * "Product detail pages per category ([slug], variant selectors)"
 * item. Calls notFound() for any slug that isn't a known Game
 * Characters product, which renders the sibling not-found.tsx in
 * this route segment.
 *
 * DATA FLOW:
 * 1. generateStaticParams pre-renders one page per known Game
 *    Characters product slug.
...
// Pre-render a static page for every known Game Characters product slug
...
export default async function GameCharacterProductPage({ params }: ProductPageProps) {
  ...
  // No matching Game Characters product for this slug — render the not-found segment
  if (!product) {
    notFound();
  }
