/**
 * FILE: app/(public)/game-characters/[slug]/page.tsx
 * ROLE: Public — Game Characters product detail page, served at
 * "/game-characters/[slug]".
 *
 * PURPOSE:
 ...
 * 404'd its "View Details" link (docs/openFindings.md, task-126).
 * Calls notFound() for any slug that isn't a published Game
 * Characters product, which renders the sibling not-found.tsx in
 * this route segment.
 *
 * DATA FLOW:
...
const CATEGORY = "game-characters" as const;
...
/**
 * getSiblings
 * Every other published Game Characters product, for the "More in
 * this category" strip. No gallery images needed here, so the
 * lighter list select is used instead of the detail select.
 */
...
export default async function GameCharacterProductPage({ params }: ProductPageProps) {
  const { slug } = await params;
  const product = await getProduct(slug);

  // No matching published Game Characters product for this slug — render the not-found segment
  if (!product) {
    notFound();
  }
