 ...
 * (scoped to the "templates" category) and renders it via the shared
 ...
 * notFound() for any slug that isn't a known Templates product, which
 * renders the sibling not-found.tsx in this route segment.
 *
 * DATA FLOW:
 * 1. generateStaticParams pre-renders one page per known Templates
 *    product slug.
...
// Pre-render a static page for every known Templates product slug
...
export default async function TemplateProductPage({ params }: ProductPageProps) {
  ...
  // No matching Templates product for this slug — render the not-found segment
  if (!product) {
    notFound();
  }
