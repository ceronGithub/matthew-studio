 ...
 * ROLE: Public — Templates product detail page, served at
 * "/templates/[slug]".
 *
 * PURPOSE:
 ...
 * 404'd its "View Details" link (same gap task-126/127/128/129 fixed
 * for the other 4 categories). Calls notFound() for any slug that
 * isn't a published Templates product, which renders the sibling
 * not-found.tsx in this route segment.
 *
 * Templates is the one category with a `variants` breakdown
 * (Static/Dynamic/Modern) that ProductDetail.tsx's variant selector
 * reads — no special-casing needed here: toPublicProduct()
 * (lib/publicProduct.ts) already includes `variants` in its mapped
 * shape whenever the row carries a real array, same as the public API.
 *
 * DATA FLOW:
...
/**
 * getSiblings
 * Every other published Templates product, for the "More in this
...
export default async function TemplateProductPage({ params }: ProductPageProps) {
  const { slug } = await params;
  const product = await getProduct(slug);

  // No matching published Templates product for this slug — render the not-found segment
  if (!product) {
    notFound();
  }
