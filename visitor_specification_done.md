...
- **Implemented (2026-09-03):** §3.1 mobile translateY simplification — added `lib/hooks/useIsMobileViewport.ts` (≤768px, same convention as `usePrefersReducedMotion.ts`) and wired it into `ScrollReveal.tsx`, so entrance distance is now 12px on mobile vs 24px on desktop/tablet, matching §3.1 exactly. Reduced-motion still overrides both to 0. This one change updates the distance for every page using `ScrollReveal` — no per-page edits needed.
- **Implemented (2026-09-03):** §3.4 page transitions — added `app/(public)/template.tsx`, a 180ms opacity fade-in wrapper. Next.js re-mounts `template.tsx` on every navigation between sibling routes (unlike `layout.tsx`), so this removes the instant hard-cut between marketing pages site-wide. No exit animation (would need a heavier `AnimatePresence`/`usePathname` setup at the layout level) — a fade-in alone satisfies §3.4's "avoid jarring instant swaps" goal.
- **Implemented (2026-09-03):** §3.5 carousel crossfade — `useMediaCarousel.ts` itself only tracks the active index (rendering is left to callers), so the fix was applied at its two actual consumers: `HeroSection.tsx` and `QuickWins.tsx` (the only components using this hook — the spec's "blog, testimonials, media" carousel framing was aspirational; those pages are grids, not carousels). Both already had an opacity-only `AnimatePresence` crossfade; added the missing `scale: 0.98 → 1` per §3.5. Both are plain `motion.div`, so `<MotionConfig reducedMotion="user">` (root layout) covers reduced-motion for free.
- **Spec fully implemented (2026-09-03):** all page-by-page items (Steps 1–6) and all cross-cutting §3 system items are now done. No known outstanding work remains against this document.
...
| 2026-09-03 | Closed out all remaining cross-cutting §3 items: added `lib/hooks/useIsMobileViewport.ts` and wired it into `ScrollReveal.tsx` for §3.1's 12px-mobile/24px-desktop entrance distance; added `app/(public)/template.tsx` for §3.4's page-transition crossfade (180ms fade-in on route change, site-wide via Next.js template re-mounting); added the missing `scale: 0.98 → 1` to the existing opacity crossfades in `HeroSection.tsx` and `QuickWins.tsx` for §3.5 (the only two actual `useMediaCarousel` consumers). **The spec is now fully implemented — no known outstanding work remains.** |

---

**Document Version:** 2.3
**Last Updated:** 2026-09-03
**Status:** Complete. All page-by-page items (Steps 1–6) and all cross-cutting §3 system items (mobile translateY distance, page transitions, carousel crossfade) are implemented. No known outstanding work remains against this document.
